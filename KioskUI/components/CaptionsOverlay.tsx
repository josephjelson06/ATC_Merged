...
export const CaptionsOverlay: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubTranscript = AgentAdapter.onTranscript(
      (text, isFinal, source) => {
    ...
    const unsubVoice = VoiceRuntime.subscribe((event) => {
      if (
        ...
        event.type === "VOICE_SESSION_ABORTED"
      ) {
        if (timeoutRef.current) clearTimeout(timeoutRef.current);
        setMode(null);
        setUserText("");
      ...
      <AnimatePresence mode="wait">
        {mode && (
          ...
          >
            ...
            <div
              className={`text-[10px] font-bold uppercase tracking-widest mb-2 opacity-80 ${
                mode === "user" ? "text-blue-300" : "text-emerald-300"
              }`}
            >
