...
import {
  ...
  RoomService,
  RoomServiceError,
} from "../../services/booking/room.service";
    ...
    RoomService.getAvailableRooms()
      ...
      .catch((error) => {
        ...
        if (error instanceof RoomServiceError) {
          ...
          setRoomsError(
            error.message || "Failed to load rooms from the server.",
          );
          return;
        }
        ...
        <div className="flex-1 flex flex-col max-w-6xl mx-auto w-full relative">
          <header className="mb-8 text-center">
            ...
            <p className="text-slate-400">
              We have prepared a selection based on your preferences.
            </p>
            {roomsError && (
              <p className="text-amber-300 text-sm mt-2">{roomsError}</p>
            )}
          </header>
          ...
          {isLoadingRooms ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center text-slate-300">
              ...
              <p className="text-sm text-slate-400">
                Please contact front desk support.
              </p>
            </div>
          ) : (
            <div
              className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 overflow-y-auto pb-24 px-4 transition-opacity ${loading ? "opacity-50 pointer-events-none" : ""}`}
            >
              {rooms.map((room: any) => (
                <RoomCard
                  key={room.id}
